...
- (BYStatResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray * _Nullable)arrayArgs orDictionary:(NSDictionary * _Nullable)dictionaryArgs orVAList:(va_list)args;
...
#pragma mark BYStatDatabase instantiation and deallocation

+ (instancetype)databaseWithPath:(NSString *)aPath {
    return BYStatDBReturnAutoreleased([[self alloc] initWithPath:aPath]);
}

+ (instancetype)databaseWithURL:(NSURL *)url {
    return BYStatDBReturnAutoreleased([[self alloc] initWithURL:url]);
}
...
- (void)dealloc {
    [self close];
    BYStatDBRelease(_openResultSets);
    BYStatDBRelease(_cachedStatements);
    BYStatDBRelease(_dateFormat);
    BYStatDBRelease(_databasePath);
    BYStatDBRelease(_openFunctions);
...
+ (NSString*)BYStatDBUserVersion {
    return @"2.7.4";
}

// returns 0x0240 for version 2.4.  This makes it super easy to do things like:
// /* need to make sure to do X with BYStatDB version 2.4 or later */
// if ([BYStatDatabase BYStatDBVersion] >= 0x0240) { … }

+ (SInt32)BYStatDBVersion {
    
    // we go through these hoops so that we only have to change the version number in a single spot.
    static dispatch_once_t once;
    static SInt32 BYStatDBVersionVal = 0;
    
    dispatch_once(&once, ^{
        NSString *prodVersion = [self BYStatDBUserVersion];
        ...
        char *e = nil;
        BYStatDBVersionVal = (int) strtoul([junk UTF8String], &e, 16);
        
    });
    
    return BYStatDBVersionVal;
}
...
static int BYStatDBDatabaseBusyHandler(void *f, int count) {
    BYStatDatabase *self = (__bridge BYStatDatabase*)f;
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)timeout {
    ...
    if (timeout > 0) {
        sqlite3_busy_handler(_db, &BYStatDBDatabaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
// but for folks who don't bother noticing that the interface to BYStatDatabase changed,
// we'll still implement the method so they don't get suprise crashes
- (int)busyRetryTimeout {
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"BYStatDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}
...
#pragma unused(i)
    NSLog(@"%s:%d", __FUNCTION__, __LINE__);
    NSLog(@"BYStatDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)closeOpenResultSets {
    ...
    NSSet *openSetCopy = BYStatDBReturnAutoreleased([_openResultSets copy]);
...
- (void)resultSetDidClose:(BYStatResultSet *)resultSet {
...
- (void)clearCachedStatements {
    ...
    for (NSMutableSet *statements in [_cachedStatements objectEnumerator]) {
        for (BYStatStatement *statement in [statements allObjects]) {
            [statement close];
        }
    }
...
- (BYStatStatement*)cachedStatementForQuery:(NSString*)query {
    
    NSMutableSet* statements = [_cachedStatements objectForKey:query];
    
    return [[statements objectsPassingTest:^BOOL(BYStatStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(BYStatStatement*)statement forQuery:(NSString*)query {
...
+ (NSDateFormatter *)storeableDateFormat:(NSString *)format {
    ...
    NSDateFormatter *result = BYStatDBReturnAutoreleased([[NSDateFormatter alloc] init]);
    result.dateFormat = format;
    result.timeZone = [NSTimeZone timeZoneForSecondsFromGMT:0];
    result.locale = BYStatDBReturnAutoreleased([[NSLocale alloc] initWithLocaleIdentifier:@"en_US"]);
    return result;
}
...
- (void)setDateFormat:(NSDateFormatter *)format {
    BYStatDBAutorelease(_dateFormat);
    _dateFormat = BYStatDBReturnRetained(format);
}
...
- (BOOL)goodConnection {
    ...
    BYStatResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
    NSLog(@"The BYStatDatabase %@ is currently in use.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
        NSAssert(false, @"The BYStatDatabase %@ is currently in use.", self);
        abort();
    }
#endif
}
...
- (BOOL)databaseExists {
    ...
    if (!_isOpen) {
        ...
        NSLog(@"The BYStatDatabase %@ is not open.", self);
        
#ifndef NS_BLOCK_ASSERTIONS
        if (_crashOnErrors) {
            NSAssert(false, @"The BYStatDatabase %@ is not open.", self);
            abort();
        }
#endif
...
- (NSError*)errorWithMessage:(NSString *)message {
    ...
    return [NSError errorWithDomain:@"BYStatDatabase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (BYStatResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:arguments orVAList:nil];
}

- (BYStatResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    BYStatStatement *statement  = 0x00;
    BYStatResultSet *rs         = 0x00;
    ...
    BYStatDBRetain(statement); // to balance the release below
    
    if (!statement) {
        statement = [[BYStatStatement alloc] init];
    ...
    rs = [BYStatResultSet resultSetWithStatement:statement usingParentDatabase:self];
    [rs setQuery:sql];
...
- (BYStatResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (BYStatResultSet *)executeQueryWithFormat:(NSString*)format, ... {
    va_list args;
...
- (BYStatResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
    return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (BYStatResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error {
    BYStatResultSet *rs = [self executeQuery:sql withArgumentsInArray:values orDictionary:nil orVAList:nil];
    if (!rs && error) {
...
- (BYStatResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    int rc                   = 0x00;
    sqlite3_stmt *pStmt      = 0x00;
    BYStatStatement *cachedStmt  = 0x00;
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[BYStatStatement alloc] init];
        
        [cachedStmt setStatement:pStmt];
        
        [self setCachedStatement:cachedStmt forQuery:sql];
        
        BYStatDBRelease(cachedStmt);
    }
...
int BYStatDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int BYStatDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(BYStatDBExecuteStatementsCallbackBlock)block {
    
    int rc;
    char *errmsg = nil;
    
    rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? BYStatDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
static NSString *BYStatDBEscapeSavePointName(NSString *savepointName) {
...
- (BOOL)startSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"savepoint '%@';", BYStatDBEscapeSavePointName(name)];
...
- (BOOL)releaseSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"release savepoint '%@';", BYStatDBEscapeSavePointName(name)];
...
- (BOOL)rollbackToSavePointWithName:(NSString*)name error:(NSError**)outErr {
    ...
    NSString *sql = [NSString stringWithFormat:@"rollback transaction to savepoint '%@';", BYStatDBEscapeSavePointName(name)];
...
- (BOOL)checkpoint:(BYStatDBCheckpointMode)checkpointMode error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:nil logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(BYStatDBCheckpointMode)checkpointMode name:(NSString *)name error:(NSError * __autoreleasing *)error {
    return [self checkpoint:checkpointMode name:name logFrameCount:NULL checkpointCount:NULL error:error];
}

- (BOOL)checkpoint:(BYStatDBCheckpointMode)checkpointMode name:(NSString *)name logFrameCount:(int *)logFrameCount checkpointCount:(int *)checkpointCount error:(NSError * __autoreleasing *)error
{
...
void BYStatDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void BYStatDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
- (void)makeFunctionNamed:(NSString *)name arguments:(int)arguments block:(void (^)(void *context, int argc, void **argv))block {
    ...
    id b = BYStatDBReturnAutoreleased([block copy]);
    
    [_openFunctions addObject:b];
    
    /* I tried adding custom functions to release the block when the connection is destroyed- but they seemed to never be called, so we use _openFunctions to store the values instead. */
#if ! __has_feature(objc_arc)
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (void*)b, &BYStatDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
    sqlite3_create_function([self sqliteHandle], [name UTF8String], arguments, SQLITE_UTF8, (__bridge void*)b, &BYStatDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}

- (BYStatSqliteValueType)valueType:(void *)value {
...
@implementation BYStatStatement
