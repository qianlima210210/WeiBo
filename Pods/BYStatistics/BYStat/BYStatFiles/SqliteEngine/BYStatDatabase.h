...
#import "BYStatResultSet.h"
#import "BYStatDatabasePool.h"

NS_ASSUME_NONNULL_BEGIN

#if ! __has_feature(objc_arc)
    #define BYStatDBAutorelease(__v) ([__v autorelease]);
    #define BYStatDBReturnAutoreleased BYStatDBAutorelease

    #define BYStatDBRetain(__v) ([__v retain]);
    #define BYStatDBReturnRetained BYStatDBRetain

    #define BYStatDBRelease(__v) ([__v release]);

    #define BYStatDBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define BYStatDBAutorelease(__v)
    #define BYStatDBReturnAutoreleased(__v) (__v)

    #define BYStatDBRetain(__v)
    #define BYStatDBReturnRetained(__v) (__v)

    #define BYStatDBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define BYStatDBDispatchQueueRelease(__v)
    #else
        #define BYStatDBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^BYStatDBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);

typedef NS_ENUM(int, BYStatDBCheckpointMode) {
    BYStatDBCheckpointModePassive  = 0, // SQLITE_CHECKPOINT_PASSIVE,
    BYStatDBCheckpointModeFull     = 1, // SQLITE_CHECKPOINT_FULL,
    BYStatDBCheckpointModeRestart  = 2, // SQLITE_CHECKPOINT_RESTART,
    BYStatDBCheckpointModeTruncate = 3  // SQLITE_CHECKPOINT_TRUNCATE
};

/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in BYStatDB are:

 - `BYStatDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<BYStatResultSet>` - Represents the results of executing a query on an `BYStatDatabase`.
 - `<BYStatDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.

 ### See also
 
 - `<BYStatDatabasePool>` - A pool of `BYStatDatabase` objects.
 - `<BYStatStatement>` - A wrapper for `sqlite_stmt`.
 
 ### External links
 
 - [BYStatDB on GitHub](https://github.com/ccgus/BYStatdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [BYStatDB mailing list](http://groups.google.com/group/BYStatdb)
 - [SQLite FAQ](http://www.sqlite.org/faq.html)
 
 @warning Do not instantiate a single `BYStatDatabase` object and use it across multiple threads. Instead, use `<BYStatDatabaseQueue>`.
...
@interface BYStatDatabase : NSObject
...
/** Create a `BYStatDatabase` object.
 
 An `BYStatDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `BYStatDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `BYStatDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    BYStatDatabase *db = [BYStatDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    BYStatDatabase *db     = [BYStatDatabase databaseWithPath:dbPath];
 ...
 @return `BYStatDatabase` object if successful; `nil` if failure.
...
/** Create a `BYStatDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `BYStatDatabase` connection is closed.
...
/** Initialize a `BYStatDatabase` object.
...
/** Initialize a `BYStatDatabase` object.
 ...
 An `BYStatDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(__attribute__((noescape)) BYStatDBExecuteStatementsCallbackBlock _Nullable)block;
...
/** Execute select statement
 ...
 Executing queries returns an `<BYStatResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[BYStatResultSet next]>`) from one record to the other.
 ...
 @return A `<BYStatResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see BYStatResultSet
 @see [`BYStatResultSet next`](<[BYStatResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (BYStatResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (BYStatResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (BYStatResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> BYStatResultSet!`
...
- (BYStatResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (BYStatResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (BYStatResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    BYStatResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
typedef NS_ENUM(int, BYStatSqliteValueType) {
    BYStatSqliteValueTypeInteger = 1,
    BYStatSqliteValueTypeFloat   = 2,
    BYStatSqliteValueTypeText    = 3,
    BYStatSqliteValueTypeBlob    = 4,
    BYStatSqliteValueTypeNull    = 5
};

- (BYStatSqliteValueType)valueType:(void *)argv;
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [BYStatDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one BYStatDB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using BYStatDatabase::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to BYStatDB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using BYStatDB you will not need to interact directly with `BYStatStatement`, but rather with `<BYStatDatabase>` and `<BYStatResultSet>` only.
...
@interface BYStatStatement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
