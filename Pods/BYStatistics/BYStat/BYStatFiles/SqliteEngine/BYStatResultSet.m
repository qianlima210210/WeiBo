...
+ (instancetype)resultSetWithStatement:(BYStatStatement *)statement usingParentDatabase:(BYStatDatabase*)aDB {
    ...
    return BYStatDBReturnAutoreleased(rs);
}
...
- (void)dealloc {
    ...
    BYStatDBRelease(_columnNameToIndexMap);
...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        return BYStatDBReturnAutoreleased([dict copy]);
    }
    else {
    ...
    if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
        ...
        if (outErr) {
            if (_parentDB) {
                ...
                *outErr = [NSError errorWithDomain:@"BYStatDatabase" code:SQLITE_MISUSE userInfo:errorMessage];
            }
