...
- (void)pushDatabaseBackInPool:(BYStatDatabase*)db;
- (BYStatDatabase*)db;

@end


@implementation BYStatDatabasePool
...
+ (instancetype)databasePoolWithURL:(NSURL *)url {
    return BYStatDBReturnAutoreleased([[self alloc] initWithPath:url.path]);
}

+ (instancetype)databasePoolWithPath:(NSString *)aPath flags:(int)openFlags {
    return BYStatDBReturnAutoreleased([[self alloc] initWithPath:aPath flags:openFlags]);
}

+ (instancetype)databasePoolWithURL:(NSURL *)url flags:(int)openFlags {
    return BYStatDBReturnAutoreleased([[self alloc] initWithPath:url.path flags:openFlags]);
}
...
- (instancetype)initWithPath:(NSString*)aPath flags:(int)openFlags vfs:(NSString *)vfsName {
    ...
    if (self != nil) {
        _path               = [aPath copy];
        _lockQueue          = dispatch_queue_create([[NSString stringWithFormat:@"BYStatdb.%@", self] UTF8String], NULL);
        _databaseInPool     = BYStatDBReturnRetained([NSMutableArray array]);
        _databaseOutPool    = BYStatDBReturnRetained([NSMutableArray array]);
...
- (void)dealloc {
    ...
    BYStatDBRelease(_databaseInPool);
    BYStatDBRelease(_databaseOutPool);
    BYStatDBRelease(_vfsName);
    
    if (_lockQueue) {
        BYStatDBDispatchQueueRelease(_lockQueue);
        _lockQueue = 0x00;
    }
...
- (void)pushDatabaseBackInPool:(BYStatDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The BYStatDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
