...
@class BYStatStatement;

/** Represents the results of executing a query on an `<BYStatDatabase>`.
...
@interface BYStatResultSet : NSObject

@property (nonatomic, retain, nullable) BYStatDatabase *parentDB;
...
/** `BYStatStatement` used by result set. */

@property (atomic, retain, nullable) BYStatStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<BYStatStatement>`
 
 @param statement A `<BYStatStatement>` to be performed
 
 @param aDB A `<BYStatDatabase>` to be used
 
 @return A `BYStatResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(BYStatStatement *)statement usingParentDatabase:(BYStatDatabase*)aDB;
